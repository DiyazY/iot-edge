...
for test_type in "${test_types[@]}"; do
    for i in {1..5}; do
        ...
        if [[ "$test_type" == "idle" ]]; then
            ...
            ansible-playbook -i inventory/${distribution}/hosts.ini ./playbooks/reliability-test.yaml --extra-vars "node_index=$random_number machine=$node_type eth_interface=$eth_interface test_type=$test_type" >> "$output_file" 2>&1
            wait

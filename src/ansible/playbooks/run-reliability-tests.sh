...
for test_type in "${test_types[@]}"; do
    for i in {1..5}; do
        ...
        if [[ "$test_type" == "idle" ]]; then
            ...
            ansible-playbook -i inventory/${distribution}/hosts.ini ./playbooks/reliability-${machine}-test.yaml --extra-vars "node_index=0" > "$output_file" 2>&1
            # TODO: gather metrics from mongo
            # seems running worker and control together a bit difficult because (1 control, 3 nodes)
        elif [[ "$test_type" == "" ]]; then
        else
